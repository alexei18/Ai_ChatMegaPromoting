...
export default function DemoSectionDesktop() {
  ...
  const hasTriggeredScrollStop = useRef(false);

  useEffect(() => {
    const handleScroll = (e: Event) => {
      if (!sectionRef.current || hasTriggeredScrollStop.current) return;
      
      const rect = sectionRef.current.getBoundingClientRect();
      
      // Check if 10 pixels from bottom of section are visible
      if (rect.bottom <= window.innerHeight + 10 && rect.bottom > window.innerHeight - 10) {
        hasTriggeredScrollStop.current = true; // Mark as triggered forever
        ...
        // Add wheel and touch event listeners to prevent scroll
  ...
  return (
    <section ref={sectionRef} className="relative bg-black overflow-hidden" style={{ minHeight: '100svh', paddingBottom: '100px' }}>
      {/* Beams Background */}
      ...
      {/* Content wrapper: left (top) and phone (bottom) */}
      <div className="relative z-20 flex flex-col" style={{ minHeight: '100svh' }}>
        {/* Top-centered left content */}
        ...
        {/* Bottom-centered phone container, never overlaps */}
        <div className="flex-shrink-0 w-full flex justify-center pb-16">
          <div className="relative h-[400px] sm:h-[500px] md:h-[600px] lg:h-[700px] w-full sm:w-[500px] md:w-[500px] lg:w-[500px] overflow-visible flex flex-col items-center mx-auto" style={{ marginBottom: '50px' }}>
            ...
            <div className="absolute top-[4%] left-1/2 -translate-x-1/2 w-[80%] h-[14%] rounded-2xl flex items-center justify-center">
